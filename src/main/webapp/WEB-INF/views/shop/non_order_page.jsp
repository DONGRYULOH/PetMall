  	  ...
  	  <script type="text/javascript">
  	  		...
  	  		$('#Payment').click(function(){    
			  	 IMP.request_pay({
			  	      ...
			  	      buyer_name : document.getElementById("new_receiver").value, //수령자 이름 
			  	      buyer_tel : document.getElementById("new_phone_first").value + '-' + document.getElementById("new_phone_mid").value + '-' + document.getElementById("new_phone_last").value, //수령자 전화번호 
			  	      buyer_addr : '서울특별시 강남구 삼성동',
			  	      buyer_postcode : '123-456',
			  	      m_redirect_url : 'https://www.naver.com' //모바일로 결제시 리다이렉트 주소(모바일로 할라면 파라미터 추가해야되나??) 
			  	  }, function(rsp) {
			  	      if ( rsp.success ) {
			  	          var msgOk = '결제가 완료되었습니다.';
			  	        msgOk += '고유ID : ' + rsp.imp_uid; // 결제 정보 조회시 필요 
			  	      ...
			  	      //<결제 완료후 주문테이블에 주문정보가 들어가야됨>
			  	       ...
			  	       // ************** <주문 세부 사항 관련 데이터들 > START ************************************
			  	  		...
			  	  		// ************** <주문 세부 사항 관련 데이터들 > END************************************
		  	  			  ...
		  	  			  var orderData = {
				  	        		...
				  	        		imp_uid : imp_uid //결제 정보 조회시 필요한 파라미터 
		  	  			  	};
			  		    ...
			  		    */
			  			$.ajax({
						    url : "${pageContext.request.contextPath}/shop/nonUserOrderInsert",
						    type : "POST",
						    contentType: "application/json; charset=utf-8",
						    data: JSON.stringify(orderData),
						    success : function(result){
						    	if(result == 1){
							     	alert("비회원 주문 테이블 입력 성공!!");
							     	//결제 완료페이지로 이동 
							     	var url = "${pageContext.request.contextPath}/shop/nonUserPaymentOk";	
							     	url = url + "?order_number=" + orderNum; //주문번호
					    			location.href = url;
						    	}else{
						    		alert("비회원 주문 정보 입력중 에러 발생!!");
						    	}
						    },
						    error : function(){
						     	alert("비회원 주문 정보 입력중 에러 발생!!");
